...
export type LiveThumbnailEntry = { channelId: number; dataUrl: string | null; sourceUrl: string | null; status: LiveThumbnailStatus; updatedAt: number; failures: number };
  ...
  return new Promise((resolve) => {
    try {
      ...
      request.onupgradeneeded = () => {
        if (!request.result.objectStoreNames.contains(STORE_NAME)) request.result.createObjectStore(STORE_NAME, { keyPath: 'channelId' });
      };
  ...
  return new Promise((resolve) => {
    try {
      ...
      request.onsuccess = () => {
        ...
        resolve(value && Date.now() - value.updatedAt <= MAX_AGE_MS ? value : null);
      };
      request.onerror = () => resolve(null);
    } catch { resolve(null); }
  });
...
export const LIVE_THUMBNAIL_MAX_AGE_MS = MAX_AGE_MS;
