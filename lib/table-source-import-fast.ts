...
function bool(v: string) {
  return ['1', 'true', 'yes', 'y', 'on'].includes(norm(v));
}
...
function sourceUrl(row: Row) {
  return val(row, 'source_url', 'sourceUrl', 'url', 'stream_url', 'channel_url');
}
...
export async function importSourceCsvFast(text: string) {
  ...
  const [channels, categories] = await Promise.all([
    prisma.channel.findMany({ select: { id: true, name: true, nameEn: true, url: true } }),
  ...
  for (const c of channels) {
    ...
    if (url) byUrl.set(url, c.id);
  }
  const categoryByName = new Map<string, number>();
  for (const c of categories) {
    categoryByName.set(norm(c.name), c.id);
    categoryByName.set(norm(c.nameEn), c.id);
  }
  ...
  for (const row of rows) {
    ...
    if (!channelId) {
      for (const name of [hints.nameEn, hints.name]) {
    ...
    if (channelId) {
      ...
      if (key) missingChannels.set(key, { row, hints });
      resolved.push({ row, channelId: 0 });
    }
  }

  let channelsCreated = 0;
  for (const { row, hints } of missingChannels.values()) {
    const name = hints.name || hints.nameEn;
    if (!name) continue;
    const nameEn = hints.nameEn || hints.name || name;
    const key = norm(nameEn);
    let existing = await prisma.channel.findFirst({ where: { OR: [{ name }, { nameEn }, { catalogKey: `csv:${key}` }] }, select: { id: true } });
    if (!existing) {
      ...
      existing = await prisma.channel.create({ data: {
        id, name, nameEn, catalogKey: `csv:${key}`, image: val(row, 'image', 'channel_image', 'logo') || null,
        url, referer: val(row, 'referer', 'channel_referer') || null, origin: val(row, 'origin', 'channel_origin') || null,
        vpn: bool(val(row, 'vpn', 'need_vpn')), iran: bool(val(row, 'iran', 'for_iran')),
        popular: BigInt(int(val(row, 'popular'))), vip: bool(val(row, 'vip', 'isvip')),
        language: val(row, 'language') || null, country: val(row, 'country') || null, platform: val(row, 'platform') || null,
        satellite: val(row, 'satellite', 'sat') || null, frequency: val(row, 'frequency', 'freq') || null,
        polarization: val(row, 'polarization') || null, symbolRate: val(row, 'symbolRate', 'symbol_rate') || null,
        serviceId: val(row, 'serviceId', 'service_id') || null, categoryId,
        categoryName: val(row, 'category_name', 'categoryName', 'category') || null,
        categoryNameEn: val(row, 'category_name_en', 'categoryNameEn') || null,
      }, select: { id: true } });
      byId.set(existing.id, existing.id);
      byName.set(norm(name), existing.id);
      byName.set(norm(nameEn), existing.id);
      byUrl.set(normalizeUrl(url), existing.id);
      channelsCreated++;
    }
    const id = existing.id;
    for (const item of resolved) {
      if (item.channelId === 0 && norm(channelHints(item.row).nameEn || channelHints(item.row).name) === key) item.channelId = id;
    }
  }
  ...
  const createData: Array<{
    id: number; channelId: number; title: string | null; url: string; referer: string | null; origin: string | null; country: string | null; vip: boolean;
  }> = [];
  let skipped = 0;
  const errors: string[] = [];
  ...
  for (let i = 0; i < valid.length; i++) {
    ...
    createData.push({
      id, channelId, title: val(row, 'title', 'source_title') || null, url,
      referer: val(row, 'referer', 'channel_referer') || null, origin: val(row, 'origin', 'channel_origin') || null,
      country: val(row, 'country') || null, vip: bool(val(row, 'vip', 'isvip')),
    });
  }
