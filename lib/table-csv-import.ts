...
async function uniqueChannelId(preferred: number, key: string) {
  ...
  let id = stableInt(key); while (await prisma.channel.findUnique({ where: { id }, select: { id: true } })) id++;
  return id;
}
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (table === 'source') {
        ...
        let channelId = int(val(r,'channelId','channel_id'));
        if (!channelId) {
          const name = val(r,'channelName','channel_name','channel','title');
          const nameEn = val(r,'channelNameEn','channel_name_en','nameEn');
          const ch = nameEn || name ? await prisma.channel.findFirst({ where: { OR: [{ name: name || undefined }, { nameEn: nameEn || undefined }] } }) : null;
          channelId = ch?.id ?? 0;
        }
        if (!channelId || !(await prisma.channel.findUnique({ where: { id: channelId }, select: { id: true } }))) throw new Error('valid channelId/channel name is required');
        ...
        let id = preferred > 0 && !(await prisma.source.findUnique({ where: { id: preferred }, select: { id: true } })) ? preferred : stableInt(`source:${channelId}:${url}`);
        while (await prisma.source.findUnique({ where: { id }, select: { id: true } })) id++;
        await prisma.source.create({ data: { id, channelId, title: val(r,'title','source_title') || null, url, referer: val(r,'referer','channel_referer') || null, origin: val(r,'origin','channel_origin') || null, country: val(r,'country') || null, vip: bool(val(r,'vip','isvip')) } });
        created++;
      }

      if (table === 'program') {
        const title = val(r,'title','program_title','name');
        const start = iso(val(r,'start','startTime','start_time')); const end = iso(val(r,'end','endTime','end_time'));
        ...
        const epg = await prisma.epgChannel.upsert({ where: { id: epgChannelId }, create: { id: epgChannelId, displayName: val(r,'channelName','channel_name') || 'Imported EPG', displayNameEn: val(r,'channelNameEn','channel_name_en') || null }, update: {} });
        const existing = await prisma.program.findUnique({ where: { id } });
        const data = { channelId: channelIdRaw || null, epgChannelId: epg.id, title, subTitle: val(r,'subTitle','subtitle') || null, description: val(r,'description','desc') || null, category: val(r,'category') || null, icon: val(r,'icon','image','logo') || null, start, end };
        existing ? await prisma.program.update({ where: { id }, data }) : await prisma.program.create({ data: { id, ...data } });
        existing ? updated++ : created++;
      }
    } catch (e) { errors.push(`Row ${line}: ${e instanceof Error ? e.message : 'invalid row'}`); }
  }
