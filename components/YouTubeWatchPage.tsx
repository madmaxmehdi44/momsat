  ...
  return <div className={styles.page}>
    <main className={`${styles.layout}${theater ? ` ${styles.theater}` : ''}`}>
      <section className={styles.mainColumn}>
        <div className={styles.watchToolbar}><Link href={`/channel/${channel.id}`} className={styles.back}>← صفحه شبکه</Link><button className={styles.headerButton} type="button" onClick={() => setTheater((v) => !v)}>{theater ? <X size={15} /> : <Tv2 size={15} />}{theater ? 'خروج از حالت سینما' : 'حالت سینما'}</button></div>
