...
export default function Player({ channel }: { channel: PlayerChannel }) {
    ...
    const run = async () => {
      try {
        ...
        hls.on(Hls.default.Events.ERROR, (_event, data) => {
          ...
          failCountRef.current += 1;
          if (data.type === 'mediaError' && failCountRef.current <= 1) {
            hls.recoverMediaError();
            return;
          }
          if (data.type === 'networkError' && failCountRef.current <= 2) {
            hls.startLoad();
            return;
          }
          if (sources.length > 1 && sourceIndex < sources.length - 1) {
  ...
  return (
    <div
      ref={containerRef}
      className={`pro-player${showControls ? ' controls-visible' : ''}`}
    ...
    >
      ...
      <div className="pro-player-top">
        ...
        <div className="pro-player-top-actions">
          {sources.length > 1 && <button className="icon-button" title="مسیرهای پخش" onClick={() => setShowSources((value) => !value)}><Settings size={18} /></button>}
        </div>
      </div>
      ...
      {showSources && sources.length > 1 && (
        <div className="pro-player-menu source-menu">
          ...
          {sources.map((source, index) => (
            <button key={source.url} className={index === sourceIndex ? 'selected' : ''} onClick={() => { activateSource(index); setShowSources(false); }}>
              <span>{source.title || `Source ${index + 1}`}</span>
              {index === sourceIndex && <span>●</span>}
            </button>
          ))}
        </div>
      )}
      ...
      {showQuality && levels.length > 0 && (
        <div className="pro-player-menu quality-menu">
          <button className={quality === -1 ? 'selected' : ''} onClick={() => { if (hlsRef.current) hlsRef.current.currentLevel = -1; setQuality(-1); setShowQuality(false); }}>Auto</button>
          {levels.map((level) => (
            <button key={level.index} className={quality === level.index ? 'selected' : ''} onClick={() => { if (hlsRef.current) hlsRef.current.currentLevel = level.index; setQuality(level.index); setShowQuality(false); }}>{level.label}</button>
          ))}
        </div>
      )}
