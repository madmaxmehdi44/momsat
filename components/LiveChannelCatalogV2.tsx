...
export default function LiveChannelCatalogV2({ channels, initialQuery = '', initialCategory = 'all' }: Props) {
  ...
  const renderChannel = (channel: Channel) => {
    ...
    return <Link href={`/channel/${channel.id}`} className={styles.card} key={channel.id} data-channel-id={channel.id} ref={(node) => { if (node) { elementsRef.current.set(channel.id, node); if (!observedRef.current.has(channel.id)) schedule(channel.id); } else elementsRef.current.delete(channel.id); }}>
