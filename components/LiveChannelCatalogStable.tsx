...
function sourceCandidates(channel: Channel): CaptureCandidate[] {
  const primary: CaptureCandidate = {
    id: null,
    title: null,
    url: channel.url,
  ...
  const sources: CaptureCandidate[] = (channel.sources || []).map((source) => ({
    id: source.id,
    title: source.title,
    url: source.url,
...
export default function LiveChannelCatalogStable({ channels, initialQuery = '', initialCategory = 'all' }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return channels.filter((channel) => {
      ...
      return (!q || haystack.includes(q))
        ...
        && (countryFilter === 'all' || channel.country === countryFilter || channel.sources.some((s) => s.country === countryFilter))
    ...
    while (runningRef.current < limit && queueRef.current.length) {
      ...
      void (async () => {
        try {
          ...
          if (cached?.dataUrl) {
            setSnapshots((current) => ({
              ...
              [channel.id]: {
                ...
                status: Date.now() - cached.updatedAt < CACHE_FRESH_MS ? (cached.status as UiStatus) : 'loading',
          ...
          if (aliveRef.current) {
            setSnapshots((current) => ({ ...current, [channel.id]: { dataUrl: entry.dataUrl, status: entry.status, cachedAt: entry.updatedAt } }));
          }
        } finally {
          runningRef.current -= 1;
          pumpRef.current();
  ...
  useEffect(() => {
    ...
    for (const [id, element] of elementsRef.current) {
      if (filtered.some((channel) => channel.id === id)) observer.observe(element);
    }
    const connection = (navigator as Navigator & { connection?: NavigatorConnection }).connection;
    ...
    return () => {
      ...
      connection?.removeEventListener?.('change', rerun);
    };
  }, [enqueue, filtered]);
  ...
  const renderChannel = (channel: Channel) => {
    ...
    return (
      <Link
        ...
        ref={(node) => {
          if (node) {
            elementsRef.current.set(channel.id, node);
            observerRef.current?.observe(node);
          } else {
            elementsRef.current.delete(channel.id);
          }
        }}
      >
  ...
  return (
    <section className={styles.catalog} dir="rtl">
      ...
      <div className={styles.stats}><span className={styles.mutedBox}>ترتیب کارت‌ها ثابت است؛ capture فقط برای کارت‌های نزدیک viewport انجام می‌شود. Snapshot کش‌شده قبل از capture نمایش داده می‌شود و در هر تغییر فیلتر صف دوباره از صفر شروع نمی‌شود.</span></div>
