...
import { importTable, type CsvTable } from '../../../../lib/table-csv-import-v2';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [categories, channels, sources, programs] = await Promise.all([prisma.category.count(), prisma.channel.count(), prisma.source.count(), prisma.program.count()]);
