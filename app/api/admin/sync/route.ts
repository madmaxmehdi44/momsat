...
function key(name: string, nameEn: string) {
  return normalizeName(nameEn) || normalizeName(name);
}
...
async function sourceId(preferred: number, channelId: number, url: string) {
  ...
  if (preferred && !(await prisma.source.findUnique({ where: { id: preferred }, select: { id: true } }))) {
    return preferred;
  }
  ...
  if (!id) id = 1;
  while (await prisma.source.findUnique({ where: { id }, select: { id: true } })) {
  ...
  try {
    ...
    for (const channel of channels) {
      ...
      if (existing) {
        ...
        await prisma.channel.create({ data: { id: channelId, ...data } });
        created++;
      }
    ...
    const skipped = sourceResults
      .filter((r) => r.status === 'disabled' || r.status === 'unconfigured')
      .map((r) => ({ adapter: r.adapter, status: r.status, error: r.error }));
    const failures = configuredFailures.map((r) => ({
      adapter: r.adapter,
      status: r.status,
      error: r.error,
    }));
    ...
    return NextResponse.json(
      {
        ...
        message: `Fetched ${channels.length} channels; ${created} created, ${updated} updated, ${sources} new sources`,
        providers: sourceResults.map((r) => ({
          adapter: r.adapter,
          status: r.status,
