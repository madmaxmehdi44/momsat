...
async function activeChannels(ids?: number[]) {
  return prisma.channel.findMany({
    ...
    select: {
      ...
      sources: { select: { id: true, title: true, url: true, referer: true, origin: true, vip: true } },
    },
    orderBy: { name: 'asc' },
  });
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const channel of channels) {
      ...
      channel.sources.forEach((source) => urls.add(source.url));
    }
    return NextResponse.json({ ok: true, count: channels.length, urls: urls.size, channels });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'Could not load streams' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const channelIds = Array.isArray(body.channelIds)
      ? body.channelIds.map(Number).filter((id) => Number.isInteger(id) && id > 0)
      : undefined;
    const requestedUrls = new Set(
      Array.isArray(body.urls)
        ? body.urls.filter((value): value is string => typeof value === 'string' && value.trim()).map((value) => value.trim())
        : [],
    );
    const channels = await activeChannels(channelIds?.length ? channelIds : undefined);
    const jobs: Array<{ channelId: number; channelName: string; url: string }> = [];
    for (const channel of channels) {
      const sources = channel.sources.length
        ? channel.sources
        : [{ id: null, title: null, url: channel.url, referer: channel.referer, origin: channel.origin, vip: false }];
      for (const source of sources) {
        if (!requestedUrls.size || requestedUrls.has(source.url.trim())) jobs.push({ channelId: channel.id, channelName: channel.name, url: source.url.trim() });
      }
    }
    ...
    async function worker() {
      while (true) {
        ...
        const result = await probeStream(job.url);
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'Stream validation failed' }, { status: 500 });
  }
}
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    const stream = await updateStreamHealth({
      channelId,
      url,
      status: 'HEALTHY',
      reason: typeof body.reason === 'string' ? body.reason : 'فعال‌سازی پس از صحت‌سنجی زنده توسط ادمین',
    });
    await prisma.channel.updateMany({
      where: { id: channelId },
      data: { archiveStatus: null, archiveNote: null, archiveSince: null, url },
    });
    return NextResponse.json({ ok: true, stream, activated: true });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'Could not activate stream' }, { status: 500 });
  }
}
